...
namespace PhoneDB.Pages.Admin.Property.Long;
using Utils 
    = Utils.Utils;
public class Create : PropertyCreator<long, LongPropertyType>
{
    ...
    public Create(PhoneDbContext context)
    {
        ...
        _validateRequest = PropertyUtils.CreatePropertyAddRequestValidator(
            ...
            (propertyTypeId) =>
            {
                return context.LongPropertyTypes.Where(item => item.Id.Equals(propertyTypeId)).ToListAsync()
                    .FirstOrNull();
            },
            ((int propertyTypeId, int sectionId) args) =>
            {
                return context.Sections.AnyAsync(item =>
                    ...
                    item.SectionType.LongPropertyTypes.Any(type => type.Id.Equals(args.propertyTypeId)))
                    .ContinueWith(item =>
                    {
            ...
            ((int propertyTypeId, int sectionId) args) =>
            {
                return context.LongProperties.AnyAsync(item =>
                    item.SectionId.Equals(args.sectionId) && item.LongPropertyTypeId.Equals(args.propertyTypeId));
            }
        );
    }
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        var item = new LongProperty()
        {
            Section = Section,
            LongData = Value,
            LongPropertyType = PropertyType
        };

        _context.LongProperties.Add(item);
