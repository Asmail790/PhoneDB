...
namespace PhoneDB.Pages.Admin.Property.Boolean;

public class Create : PropertyCreator<bool, BooleanPropertyType>
{
    ...
    public Create(PhoneDbContext context)
    {
        ...
        _validateRequest = PropertyUtils.CreatePropertyAddRequestValidator(
            ...
            (propertyTypeId) =>
            {
                return context.BooleanPropertyTypes.Where(item => item.Id.Equals(propertyTypeId)).ToListAsync()
                    .FirstOrNull();
            },
            ((int propertyTypeId, int sectionId) args) =>
            {
                return context.Sections.AnyAsync(item =>
                    ...
                    item.SectionType.BooleanPropertyTypes.Any(type => type.Id.Equals(args.propertyTypeId)))
                    .ContinueWith(item =>
                    {
            ...
            ((int propertyTypeId, int sectionId) args) =>
            {
                return context.BooleanProperties.AnyAsync(item =>
                    item.SectionId.Equals(args.sectionId) && item.BooleanPropertyTypeId.Equals(args.propertyTypeId));
            }
        );
    }
